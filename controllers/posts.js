...
router.get('/:id', async ctx => {const post = await Post.findById(ctx.params.id)
    if (post) {
      ctx.body = post
    } else {
...
router.delete('/:_id', passport.authenticate('jwt', { session: false }), async ctx => {  await Post.findOneAndRemove({
    _id: ctx.params._id,
